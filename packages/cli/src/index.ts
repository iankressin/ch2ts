...
import { parse, map, emit, type EmissionOptions, type MappingOptions } from '@ch2ts/core';
import {
  bigintPreset,
  decimalJsPreset,
  safePreset,
...
export function buildCli(): Command {
  const program = new Command()
    .name('subsquid-ch2ts')
    .description('Generate TypeScript types from ClickHouse CREATE TABLE statements')
    ...
    .option('--emit-json-schema', 'Also emit JSON Schema (when --out is used)', false)
    .option('--preset <name>', 'Use a predefined mapping preset (safe|strict|decimal.js|bigint)')
...
function resolvePreset(preset?: CliOptions['preset']): PresetOptions | undefined {
  switch (preset) {
    case 'safe':
      return safePreset;
    case 'strict':
 ...
 */
export async function runCli(argv: readonly string[], stdin?: NodeJS.ReadableStream): Promise<void> {
  ...
  const generateOnce = async () => {
    ...
    if (typeof opts.out === 'string' && opts.out.length > 0) {
      ...
      if (opts.emitJsonSchema) {
        ...
        const emitJson = (mod as unknown as { emitJsonSchema: (mapped: ReturnType<typeof map>) => string }).emitJsonSchema;
...
const isEsmMain = (() => {
  try {
    ...
    return !!meta && typeof meta.url === 'string' && meta.url === pathToFileURL(process.argv[1] ?? '').href;
  } catch {
    return false;
  }
})();
