import { Project, QuoteKind, StructureKind, VariableDeclarationKind, type PropertySignatureStructure } from 'ts-morph';
import type { EmissionOptions, EnumMember, MappedTable, TypeAst, TypeArg } from './types.js';

/** Emit TypeScript source from mapped tables (ts-morph). */
export function emit(mapped: readonly MappedTable[], options: EmissionOptions): string {
  ...
  const needsIPv4 = mapped.some((t) => t.columns.some((c) => c.tsType === 'IPv4'));
  const needsIPv6 = mapped.some((t) => t.columns.some((c) => c.tsType === 'IPv6'));
  const needsDecimal = mapped.some((t) => t.columns.some((c) => c.tsType === 'Decimal'));
  ...
  if (needsDecimal) {
    sf.addImportDeclaration({ moduleSpecifier: 'decimal.js', isTypeOnly: true, namedImports: ['Decimal'] });
  }
  if (needsIPv4) {
    sf.addTypeAlias({ isExported: true, name: 'IPv4', type: `string & { readonly __brand: 'IPv4' }` });
  }
  if (needsIPv6) {
    sf.addTypeAlias({ isExported: true, name: 'IPv6', type: `string & { readonly __brand: 'IPv6' }` });
  }
  ...
  for (const table of mapped) {
    const props: PropertySignatureStructure[] = table.columns.map((c) => ({
      ...
      name: c.name,
      type: c.tsType,
      docs: [`Original: ${c.chType}${c.comment ? ` — ${c.comment}` : ''}`],
      hasQuestionToken: false
    }));
    sf.addInterface({ isExported: true, name: table.interfaceName, properties: props });

    if (needsZod) {
      const zProps = table.columns.map((c) => `${c.name}: ${zodForTypeAst(c.typeAst, c.tsType)}`).join(', ');
      sf.addVariableStatement({
        isExported: true,
        declarationKind: VariableDeclarationKind.Const,
        declarations: [{ name: `${table.interfaceName}Schema`, initializer: `z.object({ ${zProps} })` }]
      });
    }
  }
...
function zodForTypeAst(type: TypeAst, resolvedTsType: string): string {
  switch (type.name) {
    ...
    case 'Tuple': {
      const entries = type.args.map((a, i) => `_${i}: ${zodForTypeAst(toTypeOrUnknown(a), resolvedTsType)}`).join(', ');
    ...
    case 'Enum16': {
      const keys = type.args
        .map((a) => (typeof a === 'object' && a !== null && 'key' in a ? (a as EnumMember).key : undefined))
