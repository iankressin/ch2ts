...
import { generateSource, type EmissionOptions, type MappingOptions } from './index.js';
...
describe('golden snapshots (core)', () => {
  it('simple → default', () => {
    ...
    const out = generateSource(ddl, baseMap, { emitZod: false } satisfies EmissionOptions);
  ...
  it('simple → zod', () => {
    const ddl = readFileSync(simpleSqlPath, 'utf8');
    const out = generateSource(ddl, baseMap, { emitZod: true } satisfies EmissionOptions);
