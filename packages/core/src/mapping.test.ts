...
describe('type mapping', () => {
  it('maps primitives and wrappers', () => {
    ...
    expect(mapTypeAstToTs(t('Nullable', [t('UInt64')]), opts)).toBe('bigint | null');
  });
  ...
  it('respects bigint and decimal.js presets', () => {
    const bigintOpts: MappingOptions = { ...base, int64As: 'string', camelCase: false };
    const decimalJsOpts: MappingOptions = { ...base, camelCase: false, decimal: 'decimal.js' };
    expect(mapTypeAstToTs(t('UInt64'), bigintOpts)).toBe('string');
    expect(mapTypeAstToTs(t('Decimal', [38, 10]), decimalJsOpts)).toBe('Decimal');
  });
});
