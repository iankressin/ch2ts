...
function jsonSchemaForType(type: TypeAst, resolvedTs: string): Record<string, unknown> {
  switch (type.name) {
    case 'Nullable': {
    ...
    case 'Array': {
      return { type: 'array', items: jsonSchemaForType(firstTypeArg(type), resolvedTs) };
    }
    case 'Tuple': {
      // Represented as object {_0, _1, ...}
      const props: Record<string, unknown> = {};
      type.args.forEach((a, i) => { props[`_${i}`] = jsonSchemaForType(toTypeOrUnknown(a), resolvedTs); });
    ...
    case 'Enum16': {
      const keys = type.args
        .map((a: TypeArg) => (typeof a === 'object' && a !== null && 'key' in a ? (a as EnumMember).key : undefined))
        .filter((k): k is string => typeof k === 'string');
      return keys.length > 0 ? { type: 'string', enum: keys } : { type: 'string' };
    }
    case 'Decimal':
      return { type: 'string' };
    case 'Float32':
    ...
    case 'Int64':
    case 'UInt64':
      return /\bbigint\b/.test(resolvedTs) ? { type: 'integer' } : { type: 'string' };
    case 'String':
    case 'UUID':
    case 'FixedString':
    case 'IPv4':
    ...
    case 'DateTime':
    case 'DateTime64':
      return /\bDate\b/.test(resolvedTs) ? { type: 'string', format: 'date-time' } : { type: 'string' };
    default:
      return {};
  }
}
